...
static char _bootTime[21] = {'\0'}; // Aug 02 2020 12:34:56 => 20 + \0
...
void setBootTime(){
	...
	time_t now = time(&now);
	localtime_r(&now, &btm);
	strftime(_bootTime, sizeof(_bootTime), "%b %d %Y %H:%M:%S", &btm); // http://www.cplusplus.com/reference/ctime/strftime/
	dbgprintf(ico_info, "boot time: %s", _bootTime);
}
...
char *uptime(unsigned long milli){
    ...
    mins -= hours * 60;
    hours -= days * 24;
    sprintf(_result, "%d Tag%s %2.2d:%2.2d:%2.2d", (int)days, secs < 86400 || secs >= 172800 ? "e" : "", (byte)hours, (byte)mins, (byte)secs);
    return _result;
}
